...
class ResultStorage:
    ...
    def save_result(self, result: BenchmarkResult) -> None:
        ...
        cursor.execute('''
            INSERT INTO benchmark_results (
                id, name, client_library, http_method, url, start_time, end_time,
        ...
        ''', (
            result.id, result.name, result.client_library, result.http_method, result.url,
            result.start_time.isoformat(), result.end_time.isoformat(), result.duration,
            result.requests_count, result.requests_per_second, result.avg_response_time,
            result.min_response_time, result.max_response_time, result.p95_response_time,
            result.p99_response_time, result.cpu_usage_avg, result.memory_usage_avg,
            json.dumps(result.network_io), result.error_count, result.error_rate,
            result.concurrency_level, json.dumps(result.config_snapshot)
        ))
    ...
    def _row_to_benchmark_result(self, row: tuple) -> BenchmarkResult:
        ...
        return BenchmarkResult(
            ...
            requests_per_second=row[9],
            avg_response_time=row[10],
            min_response_time=row[11],
            max_response_time=row[12],
            p95_response_time=row[13],
            p99_response_time=row[14],
            cpu_usage_avg=row[15],
            memory_usage_avg=row[16],
            network_io=json.loads(row[17]),
            error_count=row[18],
            error_rate=row[19],
            concurrency_level=row[20],
            config_snapshot=json.loads(row[21])
        )