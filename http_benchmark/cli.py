...
def main():
    ...
    parser.add_argument("--client", required=False, choices=["requests", "requestx", "httpx", "aiohttp", "urllib3", "pycurl"],
                        help="HTTP client library to use (required unless --compare is used)")
    parser.add_argument("--method", default="GET", choices=["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS", "STREAM"],
...
def compare_clients(args) -> None:
    ...
    for result in results:
        print(f"{result.client_library:<12} {result.requests_per_second:<10.2f} "
